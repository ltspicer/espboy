...
bool inversDisplayJumper = false; // For testing without set/remove jumper
...
// 20x20 Taste #
const byte buttonHashBitmap20x20[] PROGMEM = {
  0b11111111, 0b11111111, 0b11110000,
  0b11111111, 0b11111111, 0b11110000,
  0b11000000, 0b00000000, 0b00110000,
  0b11000010, 0b00000100, 0b00110000,
  0b11000010, 0b00000100, 0b00110000,
  0b11000010, 0b00000100, 0b00110000,
  0b11011111, 0b11111111, 0b10110000,
  0b11011111, 0b11111111, 0b10110000,
  0b11000010, 0b00000100, 0b00110000,
  0b11000010, 0b00000100, 0b00110000,
  0b11000010, 0b00000100, 0b00110000,
...
static byte backupStack[10][20][20];

byte spielfeld[20][20];
...
byte vierGewinntMaxDepth = 2;       // Maximale Tiefe des Minimax
const byte order[7] = {3, 2, 4, 1, 5, 0, 6};

// Für Reversi
int xPos = 0;
int yPos = 0;

byte reversiLevel = 3;              // Maximale Tiefe des Minimax
...
int tttMoves = 1;
bool tttCpuTurn = true;        // Wer beginnt?
int tttboard[]={0,0,0,0,0,0,0,0,0}; // holds position data 0 is blank, 1 human, 2 is computer
byte xLinks = 0;
byte yOben  = 0;
uint16_t breiteHoehe = 0;
uint16_t vertical1   = 0;
uint16_t vertical2   = 0;
uint16_t horizontal1 = 0;
uint16_t horizontal2 = 0;
...
void gameControllInfo(String title, String infoLine1, String infoLine2, bool showKey5) {
  ...
  myDFPlayer.reset();
  delay(500);
  myDFPlayer.volume(loadValueFromLittleFS("volume", 20));
}
...
void setup() {
  ...
  // Grosses oder kleines Display? (Jumper PCF2-Pin1 oder Pin7 --> GND)
  bool readSmallDisplay = pcf2.read(1) && pcf2.read(7);
  if (inversDisplayJumper) {
    readSmallDisplay = !readSmallDisplay;
  }
  ...
  if (!passwd.isEmpty()) {
    ...
    if (version != inoVersion) {  // Wenn LittleFS nicht der compilierten Version entspricht
      ...
      saveValueToLittleFS("info", infoFile);
    }
  }
end:
  ...
  if (dfPlayerOK) {
    myDFPlayer.reset();
    delay(800);
    myDFPlayer.volume(loadValueFromLittleFS("volume", 20));  // Lautstärke setzen
    delay(200);
    ...
    if (pressedKey >= '1' && pressedKey <= '9') {
      ...
      switch (auswahl) {
        ...
        case 7:
          ...
          break;
        case 8:
          Serial.println(F("Nr. 8 gewählt"));
          break;
        case 9:
          Serial.println(F("Nr. 9 gewählt"));
          break;
        default:
...
void settingsMenu() {
  ...
  while (true) {
    ...
    if (pressedKey >= '1' && pressedKey <= '9') {
      ...
      switch (auswahl) {
        case 1:
          Serial.println(F("4 gewinnt Level gewählt"));
          readNumberFromKeypad("Level", "4gewinntlevel", 2, 4, 1);
          displaySettingsMenu();
          break;
        case 2:
          Serial.println(F("Reversi Level gewählt"));
          readNumberFromKeypad("Level", "reversilevel", 3, 6, 1);
          displaySettingsMenu();
          break;
        case 3:
        ...
        case 5:
          ...
          readNumberFromKeypad("Volume", "volume", 20, 30, 0);
          displaySettingsMenu();
          break;
        case 6:
...
// User Werteingabe
void readNumberFromKeypad(String titel, String filename, int standardWert, int maxInput, int minInput) {
  byte value = loadValueFromLittleFS(filename, standardWert);
  displayKeyInput(titel, value);
  char key;
  while (true) {
    key = scanKeypad();
    autoPowerOff();
    if (key == 'A') {
      saveValueToLittleFS(filename, String(value));  // Speichern, wenn Eingabe abgeschlossen
      break;
    }
    if (key == '*') {
      value = max(minInput, value - 1);  // Decrement
      displayKeyInput(titel, value);
      myDFPlayer.volume(value);
    } else if (key == 'D') {
      value = min(maxInput, value + 1);  // Increment
      displayKeyInput(titel, value);
      myDFPlayer.volume(value);
    }
...
void readMinesNumberFromKeypad() {
  int value = loadValueFromLittleFS("mines", 10);
  displayMines(value);
  char key;
  while (true) {
    ...
    if (key == 'A') {
      if (value > 0 && value < 80) {
        saveValueToLittleFS("mines", String(value));  // Speichern, wenn Eingabe abgeschlossen
        break;
      } else {
...
void displayButtonAndText(uint16_t iconX, byte iconY, String text, char taste) {
  ...
  if (taste == '#') {
    if (smallDisplay) {
      ...
      tft->drawBitmap(iconX, iconY - 1, buttonHashBitmap20x20, 20, 20, iconColor);
    }
  }
...
// Volume/Reversi Level Eingabe auf Display
void displayKeyInput(String titel, byte wert) {
  tft->fillScreen(bgColor);
  tft->setCursor(1, 1);
  tft->setTextColor(textColor);
  tft->setTextSize(smallDisplay ? 2 : 4);
  tft->println(titel + ": " + String(wert));
  tft->setTextSize(smallDisplay ? 1 : 2);
...
void displayMenu() {
  ...
  tft->setTextColor(textColor);
  displayButtonAndText(1, smallDisplay ? 23 : 36, "Reversi", '1');
  displayButtonAndText(1, smallDisplay ? 34 : 58, "4 gewinnt", '2');
  displayButtonAndText(1, smallDisplay ? 45 : 80, "Mini Mastermind", '3');
  displayButtonAndText(1, smallDisplay ? 56 : 102, "Minesweeper", '4');
  displayButtonAndText(1, smallDisplay ? 67 : 124, "Senso", '5');
  displayButtonAndText(1, smallDisplay ? 78 : 146, "Snake", '6');
  displayButtonAndText(smallDisplay ? 66 : 132, smallDisplay ? 67 : 124, "---", '8');
  displayButtonAndText(smallDisplay ? 66 : 132, smallDisplay ? 78 : 146, "---", '9');
  displayButtonAndText(1, smallDisplay ? 89 : 168, "Tic Tac Toe", '7');
  displayButtonAndText(1, smallDisplay ? 106 : 198, "Hiscores", '*');
  displayButtonAndText(1, smallDisplay ? 117 : 220, "Einstellungen", '#');
}
...
void displaySettingsMenu() {
  ...
  displayButtonAndText(1, smallDisplay ? 22 : 44, "4 gewinnt Level", '1');
  displayButtonAndText(1, smallDisplay ? 33 : 66, "Reversi Level", '2');
  displayButtonAndText(1, smallDisplay ? 44 : 88, "Minesweeper Minen", '3');
  displayButtonAndText(1, smallDisplay ? 55 : 110, "Hiscores loeschen", '4');
  displayButtonAndText(1, smallDisplay ? 66 : 132, "Volume", '5');
  displayButtonAndText(1, smallDisplay ? 77 : 154, "WLAN", '6');
  displayButtonAndText(1, smallDisplay ? 88 : 176, "Changelog", '7');
...
// Wert in LittleFS schreiben
void saveValueToLittleFS(String filename, String value) {
  if (!LittleFS.begin()) {
    Serial.println(F("LittleFS Mount Failed"));
    return;
  }
  ...
  file.print(value);
  file.close();
  Serial.println(filename + " saved: " + value);
}

// Wert aus LittleFS holen
byte loadValueFromLittleFS(String filename, byte standardwert) {
  if (!LittleFS.begin()) {
    Serial.println(F("LittleFS Mount Failed"));
    return standardwert;  // Standardwert, falls Mount fehlschlägt
  }
  if (!LittleFS.exists("/" + filename + ".txt")) {
    Serial.println(F("File not found, using default value"));
    return standardwert;  // Standardwert, falls Datei nicht existiert
  }
...
void vierGewinnt() {
  ...
  vierGewinntMaxDepth = loadValueFromLittleFS("4gewinntlevel", 2);
...
void computerZug() {
  ...
  for (byte spalte = 0; spalte < SPALTEN; spalte++) {
    if (istGueltigerZug(spalte)) {
      ...
      int zugWert = minimaxVierGewinnt(vierGewinntMaxDepth, false);
...
int minimaxVierGewinnt(int tiefe, bool maximierend) {
  yield();
  int bewertung = vierGewinntBewerten();
...
int vierGewinntBewerten() {
  ...
  for (byte zeile = 0; zeile < ZEILEN; zeile++) {
      if (spielfeld[3][zeile] == COMPUTER) score += 3;
      if (spielfeld[3][zeile] == SPIELER) score -= 3;
  }
  ...
  for (byte zeile = 0; zeile < ZEILEN; zeile++) {
    for (byte spalte = 0; spalte < SPALTEN - 3; spalte++) {
      ...
      if (computerSteine > 0 && spielerSteine == 0) {
        // Nur Computer-Steine in diesem 4er-Fenster
        if (computerSteine == 3) score += 50;   // fast gewonnen
        else if (computerSteine == 2) score += 10;
        else if (computerSteine == 1) score += 1;
      }

      if (spielerSteine > 0 && computerSteine == 0) {
        // Nur Spieler-Steine in diesem 4er-Fenster
        if (spielerSteine == 3) score -= 80;   // Gegner droht zu gewinnen → stärker bestrafen
        else if (spielerSteine == 2) score -= 15;
        else if (spielerSteine == 1) score -= 1;
      }
    }
  }
...
void reversi() {
  ...
  reversiLevel = loadValueFromLittleFS("reversilevel", 3);
...
int64_t bewerteSpielfeld() {
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      if (spielfeld[i][j] == COMPUTER) wert += WEIGHT[i][j];
      else if (spielfeld[i][j] == SPIELER) wert -= WEIGHT[i][j];
    }
  }
    ...
    for (int x = 0; x < SPALTEN; x++) {
      for (int y = 0; y < ZEILEN; y++) {
        ...
        if (istGueltigerZugReversi(x, y, COMPUTER)) {
          zugGefunden = true;
          memcpy(backupStack[tiefe], spielfeld, sizeof(spielfeld));
          macheZugReversi(x, y, COMPUTER);

          int64_t wert = minimaxReversi(tiefe - 1, false, alpha, beta);
          memcpy(spielfeld, backupStack[tiefe], sizeof(spielfeld));
  ...
  for (byte x = 0; x < SPALTEN; x++) {
    for (byte y = 0; y < ZEILEN; y++) {
      if (istGueltigerZugReversi(x, y, COMPUTER)) {
        ...
        // Backup des gesamten Spielfelds
        memcpy(backupStack[tiefe], spielfeld, sizeof(spielfeld));
  ...
  while (true) {
    if (spielerDran) {
      ...
      if (kannSpielerZugMachen(COMPUTER)) {
        ...
        Zug besterZug = findeBestenZugReversi(reversiLevel);
...
void minesweeper() {
  NUM_MINES = loadValueFromLittleFS("mines", 10);
  bgColor = ST77XX_BLACK;
  iconColor = ST77XX_WHITE;
  textColor = ST77XX_WHITE;
...
void sensoInitGame() {
  ...
  myDFPlayer.play(21);
  delay(2500);
  myDFPlayer.volume(loadValueFromLittleFS("volume", 20) + 5);
  delay(200);
  myDFPlayer.stop();
  delay(200);
}
...
//////////////////////////////////////////////
// Funktionen und Hauptroutine für Tic Tac Toe
//////////////////////////////////////////////
...
void ticTacToe(){
  ...
  tft->setTextSize(smallDisplay ? 1 : 2);
  tft->setCursor(8, smallDisplay ? 20 : 40);
  tft->print(F("Wer beginnt?"));

  displayButtonAndText(8, smallDisplay ? 30 : 60, "Computer", 'C');
  displayButtonAndText(8, smallDisplay ? 42 : 84, "Du", 'D');
  displayButtonAndText(8, smallDisplay ? 60 : 120, "Spiel abbrechen", 'A');
  ...
  if (!tttPlayGame()) {
    ...
    tft->setTextSize(smallDisplay ? 1 : 2);
    tft->setCursor(10, smallDisplay ? 56 : 104);
    tft->print(F("Abgebrochen"));
    delay(3000);
  }
  // Spiel fertig. Zurück ins Menu
}

void tttDrawGameScreen() {
  tft->fillScreen(bgColor);
  xLinks = smallDisplay ? 20 : 41;
  yOben  = smallDisplay ? 4 : 2;
  breiteHoehe = smallDisplay ? 120 : 232;
  vertical1   = breiteHoehe/3   + xLinks;
  vertical2   = breiteHoehe/3*2 + xLinks;
  horizontal1 = breiteHoehe/3   + yOben;
  horizontal2 = breiteHoehe/3*2 + yOben;

  tft->drawRect(0,0,TFT_HEIGHT-1,TFT_WIDTH-1,textColor);
  tttDrawVerticalLine(vertical1);
  tttDrawVerticalLine(vertical2);
  tttDrawHorizontalLine(horizontal1);
  tttDrawHorizontalLine(horizontal2);

  if (smallDisplay) {
    tft->setTextSize(1);
    tft->setCursor(38, 17); tft->print(F("1"));
    tft->setCursor(76, 17); tft->print(F("2"));
    tft->setCursor(115, 17); tft->print(F("3"));
    tft->setCursor(38, 58); tft->print(F("4"));
    tft->setCursor(76, 58); tft->print(F("5"));
    tft->setCursor(115, 58); tft->print(F("6"));
    tft->setCursor(38, 99); tft->print(F("7"));
    tft->setCursor(76, 99); tft->print(F("8"));
    tft->setCursor(115, 99); tft->print(F("9"));
    displayButtonAndText(147, 58, "", 'A');
  } else {
    tft->setTextSize(2);
    tft->setCursor(76, 33); tft->print(F("1"));
    tft->setCursor(153, 33); tft->print(F("2"));
    tft->setCursor(230, 33); tft->print(F("3"));
    tft->setCursor(76, 112); tft->print(F("4"));
    tft->setCursor(153, 112); tft->print(F("5"));
    tft->setCursor(230, 112); tft->print(F("6"));
    tft->setCursor(76, 191); tft->print(F("7"));
    tft->setCursor(153, 191); tft->print(F("8"));
    tft->setCursor(230, 191); tft->print(F("9"));
    displayButtonAndText(296, 109, "", 'A');
  }
}

void tttDrawHorizontalLine(uint16_t y) {
  byte dicke = smallDisplay ? 4 : 8;
  tft->fillRect(xLinks,y-dicke/2,breiteHoehe,dicke,textColor);
}

void tttDrawVerticalLine(uint16_t x) {
  byte dicke = smallDisplay ? 4 : 8;
  tft->fillRect(x-dicke/2,yOben,dicke,breiteHoehe,textColor);
}

void tttDrawGameOverScreen(byte winner){
  tft->setTextSize(smallDisplay ? 1 : 2);
  tft->setCursor(10, smallDisplay ? 56 : 104);
  ...
  if(winner == 2) {
    ...
    myDFPlayer.play(14);
    tft->setTextColor(ST77XX_RED, bgColor);
    tft->print(F("Computer hat gewonnen"));
  }
  delay(3800);
  myDFPlayer.reset();
}
...
bool tttPlayGame() {
  ...
  do {
    if (tttCpuTurn) {
      tttComputerMove();
      tttPrintBoard();
      winner = tttCheckWinner();
    } else {
      if (!tttPlayerMove()) return false;   // Wenn abgebrochen mit 'A'
      tttPrintBoard();
      winner = tttCheckWinner();  
    }
    tttCpuTurn = !tttCpuTurn; // Spielerwechsel
    tttMoves++;
  } while (winner==0 && tttMoves<10);

  delay(500);
  if(winner == 1) {
    Serial.println("Spieler hat gewonnen");
    tttDrawGameOverScreen(winner);
    delay(3000);
    return true;
  } else if(winner == 2) {
    Serial.println("Computer hat gewonnen");
    tttDrawGameOverScreen(winner);
    delay(3000);
    return true;
  } else {
...
int tttCheckOpponent() {
  if(tttboard[0]==1 && tttboard[1]==1 && tttboard[2]==0)
  return 2;
  else if(tttboard[0]==1 && tttboard[1]==0 && tttboard[2]==1)
  return 1;
  else if (tttboard[1]==1 && tttboard[2]==1 && tttboard[0]==0)
  return 0;
  else if (tttboard[3]==1 && tttboard[4]==1 && tttboard[5]==0)
  return 5;
  else if (tttboard[4]==1 && tttboard[5]==1&& tttboard[3]==0)
  return 3;
  else if (tttboard[3]==1 && tttboard[4]==0&& tttboard[5]==1)
  return 4;
  else if (tttboard[1]==0 && tttboard[4]==1&& tttboard[7]==1)
  return 1;
  else
  return 100;
}

int tttFindWinningMove(byte player) {
  for (int i = 0; i < 8; i++) {
    byte a = tttWins[i][0];
    byte b = tttWins[i][1];
    byte c = tttWins[i][2];

    int count = (tttboard[a] == player) +
                (tttboard[b] == player) +
                (tttboard[c] == player);

    if (count == 2) {
      if (tttboard[a] == 0) return a;
      if (tttboard[b] == 0) return b;
      if (tttboard[c] == 0) return c;
    }
  }
  return -1;
}

bool tttCreatesFork(byte player, int pos) {
  if (tttboard[pos] != 0) return false;
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (count == 2 &&
        (tttboard[a] == 0 || tttboard[b] == 0 || tttboard[c] == 0)) {
      winningLines++;
    }
  }
...
void tttComputerMove() {
  ...
  // 1. Kann CPU gewinnen?
  move = tttFindWinningMove(2);
  if (move >= 0) goto MAKE_MOVE;

  // 2. Kann Gegner gewinnen? → blocken
  move = tttFindWinningMove(1);
  if (move >= 0) goto MAKE_MOVE;

  // 3. Kann CPU Fork erzeugen?
  for (int i = 0; i < 9; i++) {
    if (tttCreatesFork(2, i)) {
      move = i;
      goto MAKE_MOVE;
    }
  }

  // 4. Kann Gegner Fork erzeugen? → verhindern
  for (int i = 0; i < 9; i++) {
    if (tttCreatesFork(1, i)) {
      move = i;
      goto MAKE_MOVE;
    }
  }

  // 5. Mitte nehmen (aber nicht immer)
  if (tttboard[4] == 0 && random(100) < 40) {
      move = 4;
      goto MAKE_MOVE;
  }
...
void tttDrawCircle(uint16_t x, uint16_t y) {
  byte thickness = smallDisplay ? 4 : 8;
  byte radius = smallDisplay ? 16 : 32;
  for (int i = 0; i < thickness; i++) {
    tft->drawCircle(x, y, radius - i, ST77XX_RED);
  }
}

void tttDrawX(uint16_t x, uint16_t y) {
  byte size = smallDisplay ? 14 : 28;
  byte thickness = smallDisplay ? 5 : 9;
  for (int i = -thickness/2; i <= thickness/2; i++) {
      tft->drawLine(x - size + i, y - size, x + size + i, y + size, ST77XX_BLUE);
      tft->drawLine(x - size + i, y + size, x + size + i, y - size, ST77XX_BLUE);
  }
}

void tttDrawCpuMove(int move) {
  if (smallDisplay) {
    switch(move) {
      case 0: tttDrawCircle(39,20);   break;
      case 1: tttDrawCircle(79,20);   break;
      case 2: tttDrawCircle(118,20);  break;
      case 3: tttDrawCircle(39,62);   break;
      case 4: tttDrawCircle(79,62);   break;
      case 5: tttDrawCircle(118,62);  break;
      case 6: tttDrawCircle(39,103);  break;
      case 7: tttDrawCircle(79,103);  break;
      case 8: tttDrawCircle(118,103); break;
    }
  } else {
    switch(move) {
      case 0: tttDrawCircle(79,37);   break;
      case 1: tttDrawCircle(156,37);  break;
      case 2: tttDrawCircle(233,37);  break;
      case 3: tttDrawCircle(79,116);  break;
      case 4: tttDrawCircle(156,116); break;
      case 5: tttDrawCircle(233,116); break;
      case 6: tttDrawCircle(79,195);  break;
      case 7: tttDrawCircle(156,195); break;
      case 8: tttDrawCircle(233,195); break;
    }
  }
}

void tttDrawPlayerMove(int move) {
  if (smallDisplay) {
    switch(move) {
      case 0: tttDrawX(39,20);   break;
      case 1: tttDrawX(79,20);   break;
      case 2: tttDrawX(118,20);  break;
      case 3: tttDrawX(39,62);   break;
      case 4: tttDrawX(79,62);   break;
      case 5: tttDrawX(118,62);  break;
      case 6: tttDrawX(39,103);  break;
      case 7: tttDrawX(79,103);  break;
      case 8: tttDrawX(118,103); break;
    }
  } else {
    switch(move) {
      case 0: tttDrawX(79,37);   break;
      case 1: tttDrawX(156,37);  break;
      case 2: tttDrawX(233,37);  break;
      case 3: tttDrawX(79,116);  break;
      case 4: tttDrawX(156,116); break;
      case 5: tttDrawX(233,116); break;
      case 6: tttDrawX(79,195);  break;
      case 7: tttDrawX(156,195); break;
      case 8: tttDrawX(233,195); break;
    }
  }
}
...
byte tttCheckWinner() {
  ...
  if (tttboard[3]==1 && tttboard[4]==1 && tttboard[5]==1) { 
    winner=1; 
  }
  if (tttboard[6]==1 && tttboard[7]==1 && tttboard[8]==1) { 
    winner=1; 
  }  
  if (tttboard[0]==1 && tttboard[3]==1 && tttboard[6]==1) { 
    winner=1; 
  }
  if (tttboard[1]==1 && tttboard[4]==1 && tttboard[7]==1) { 
    winner=1; 
  }
  if (tttboard[2]==1 && tttboard[5]==1 && tttboard[8]==1) { 
    winner=1; 
  }  
  if (tttboard[0]==1 && tttboard[4]==1 && tttboard[8]==1) { 
    winner=1; 
  }
  if (tttboard[2]==1 && tttboard[4]==1 && tttboard[6]==1) { 
    winner=1; 
  }
  // crosses win?
  ...
  if (tttboard[3]==2 && tttboard[4]==2 && tttboard[5]==2) { 
    winner=2; 
  }
  if (tttboard[6]==2 && tttboard[7]==2 && tttboard[8]==2) { 
    winner=2; 
  }  
  if (tttboard[0]==2 && tttboard[3]==2 && tttboard[6]==2) { 
    winner=2; 
  }
  if (tttboard[1]==2 && tttboard[4]==2 && tttboard[7]==2) { 
    winner=2; 
  }
  if (tttboard[2]==2 && tttboard[5]==2 && tttboard[8]==2) { 
    winner=2; 
  }  
  if (tttboard[0]==2 && tttboard[4]==2 && tttboard[8]==2) { 
    winner=2; 
  }
  if (tttboard[2]==2 && tttboard[4]==2 && tttboard[6]==2) { 
    winner=2; 
  }
  return winner;
}
